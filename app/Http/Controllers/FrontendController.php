...
{
    // Add new language
    public function home_index_local(Request $request, $local)
    {
    ...
    public function home_index_link_local(Request $request, $local, $name)
    {
        // dd($local, $name);
        ...
        if ($linkd) {
            $page1 = Post::where('id', '=', optional($linkd->pages->first())->id)
                ->where('post_type', '=', 'page')
                ->where('language', '=', $local)
                ->first();

            $page = $page1;
        } else {
            $page = Post::where('id', '=', optional($linkd1->pages->first())->id)
                ->where('post_type', '=', 'page')
                ->where('language', '=', $local)
                ->first();
        }
